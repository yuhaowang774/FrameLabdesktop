...
// 内置模板 47 套（2026-09-16 模板库甄选：画廊两轮打标后删 37 套淘汰款、5 套重组款晋升为正式模板；
// 此前：2026-09-12 扩充 + 09-14 FrameElf 样式学习批 + 同日引擎扩展批 + 09-14 去重删 7 套），
// 按设计语言分组（group 字段，弹窗分类侧栏过滤）：
...
import { PROMOTED_TEMPLATES } from '../core/promotedTemplates'
// 多行文字块族（第 4 批 20 套）：由 scripts/gen-text-block-templates.mjs 生成的字面量
import { TEXT_BLOCK_TEMPLATES } from '../core/textBlockTemplates'
...
const BUILTIN: FrameTemplate[] = [
      ...
      infoLayer: {
        ...
        elements: [
          ...
          },
          {
            ...
            type: 'text',
            enable: true,
            // 书脊竖排：贴内容区左缘内收 30（旧 x:-638 走 center 锚点已滑出画布左缘、成片不可见）
            x: 30,
            y: 0,
            anchorX: 'left',
            anchorY: 'center',
    ...
    config: {
      ...
      infoLayer: {
        ...
        elements: [
          ...
          {
            // 内框竖线（左）：贴内容区左缘（x=0 即照片左缘）。rotate 90 时线自锚点向下延伸，
            // 故锚画布顶边（anchorY:'top'）；定长 2800 ≥ 最高画布（9:16 竖图 + 底部签名区），
            // 溢出由画布裁掉，任何画幅都贯通全高（旧值 x:1063 走 center 锚点已出画布右缘）。
            id: 'el-pc2-frame-l',
            type: 'divider',
            enable: true,
            x: 0,
          ...
          {
            // 内框竖线（右）：贴内容区右缘，自画布底边向上延伸，与左线合成闭合内框（定长理由同左线）
            id: 'el-pc2-frame-r',
            type: 'divider',
            enable: true,
            x: 0,
    ...
    config: {
      ...
      infoLayer: {
        ...
        elements: [
          ...
          {
            // 金框竖线（左）：贴内容区左缘内收 60，自画布顶边向下延伸（rotate 90 → 线沿锚点向下）；
            // 定长 2800 保证竖图画布也贯通全高，溢出由画布裁掉
            id: 'el-gf-line-l',
            type: 'divider',
            enable: true,
            x: 60,
          ...
          {
            // 金框竖线（右）：贴内容区右缘内收 60（right 锚点用负值向内收），自画布底边向上延伸
            id: 'el-gf-line-r',
            type: 'divider',
            enable: true,
            x: -60,
    ...
    config: {
      ...
      infoLayer: {
        ...
        elements: [
          ...
          {
            // 木牌内框竖线（左）：贴内容区左缘内收 31（与横线 1330/1392 的 31 留边同宽）；
            // 定长 2800 保证竖图画布也贯通全高，溢出由画布裁掉
            id: 'el-ps-frame-l',
            type: 'divider',
            enable: true,
          ...
          {
            // 木牌内框竖线（右）：贴内容区右缘内收 31，自画布底边向上延伸（定长理由同左线）
            id: 'el-ps-frame-r',
            type: 'divider',
            enable: true,
            x: -31,
  ...
  // ===== 第 1 批（2026-09-16）：按《FrameElf 语料范式清单》A 档自主重写 6 套 =====
  // 合规：仅沿用「版式骨架」（铺满/底带厚度/行数/锚点）的范式，命名、说明、文案、配色、
  // 几何数值全部自定，不使用语料里的素材、字体与原文案。
  {
    id: 'm_wm_signature_edge',
    group: '水印署名',
    name: '浮层署名·左缘单行',
    desc: '照片铺满无边框，左缘一行签名字标浮在画面上，极简的无框署名款',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  },
  {
    ...
    group: '水印署名',
    name: '浮层日期·右下角',
    desc: '方形画幅铺满照片，右下角一行拍摄日期，安静的角落时间戳',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_border_left_two',
    group: '经典',
    name: '白边·左对齐双行',
    desc: '白底等宽边框配底部加宽带，左对齐机型与日期两行大字，干净的展陈白框',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_border_params_four',
    group: '经典',
    name: '白边·参数四行',
    desc: '竖幅白底厚底带，机型 / 参数 / 镜头 / 日期四行左对齐，参数区用等宽字',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_palette_note_band',
    group: '多彩色卡',
    name: '色卡·底部参数带',
    desc: '照片取色色卡配 hex 色号压在底部厚带，顶部一行大写字标，参数与日期随行',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_ticket_city_strip',
    group: '纸品印刷',
    name: '票根·地名字标横版',
    desc: '米白票据卡配砖红外圈，照片内缩压边，左下地名大写字标与日期、右下编号',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  // ===== 第 2 批（2026-09-16）：色卡范式族（magazine 取色色卡）自主重写 5 套 =====
  // 骨架来源：语料 06_ColorWalk / 07_多彩边框 / 22_色卡边框（底带 + 取色色卡条 + 参数行）；
  // 文案（标题/说明）与配色、带厚、画幅全部自定。
  {
    id: 'm_palette_min_edge',
    group: '多彩色卡',
    name: '色卡·窄白边',
    desc: '近白窄边配中厚底带，底带只留一条照片取色色卡，安静的极简色卡款',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_palette_ink_band',
    group: '多彩色卡',
    name: '色卡·墨底参数',
    desc: '墨底厚带上排布取色色卡与 hex 色号，右侧机型与参数行右对齐',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_palette_square_card',
    group: '多彩色卡',
    name: '色卡·方形白卡',
    desc: '方形白卡配底部色卡带，标题色卡与 hex 色号并列，画册页式的干净排版',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_palette_blur_band',
    group: '多彩色卡',
    name: '色卡·模糊延展带',
    desc: '照片色调延伸的模糊底带上排取色色卡与参数，色卡与画面同源的柔和款',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'blur',
      ...
      showDate: true,
      dateFormat: 'date',
      cameraModelColor: '#FFFFFF',
      exifTextColor: '#FFFFFF',
  ...
  {
    id: 'm_palette_wide_date',
    group: '多彩色卡',
    name: '色卡·宽幅白',
    desc: '宽幅白卡配底部色卡与参数行，副标题带拍摄日期，横向排版的色卡页',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  // ===== 第 3 批（2026-09-16）：多行文字块范式族（自由元素拼装）5 套 =====
  // 骨架来源：语料「5~8 行文字块」类（08_经典水印 / 09_白边边框 / 16_无边水印 / 20_节日边框 等），
  // 按下方《多行文字块规范》统一拼装；文案、配色、字号全自定。
  //
  // 【多行文字块规范】（信息层自由元素 infoLayer，设计 px / 1200 宽基准）
  // · 字号档：字标 26~30（700）/ 主值 18~20（500~600）/ 小字 14~16（400~500）
  // · 行距：逐行元素 pitch 24~36 且同块一致；静态多行文本用 lineHeight 1.6~1.9
  // · 字距：拉丁小字 2~4，大写字标 4~6，中文 1~2
  // · 锚点：左下 = anchorX left + anchorY bottom + x 正 / y 负（元素原点 = 该行垂直中心）；
  //        右对齐块 = anchorX right + align right + x 负；全幅浮层块一律加 shadow
  // · 压字规则：落在照片 / 模糊底上 → shadow: true；落在纯色带里 → 不投影
  // · 数值行一律用 exif 元素（{model}/{focal}/{aperture}/{shutter}/{iso}/{lens}/{gps}/{date}），
  //   缺字段自动跳过，避免出现空行占位
  {
    id: 'm_spec_five_lines',
    group: '水印署名',
    name: '规格卡·五段参数',
    desc: '白底中厚底带，左对齐五行规格：机型 / 焦段光圈 / 快门感光 / 镜头 / 日期，参数区等宽字',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_panorama_seven_lines',
    group: '创意排版',
    name: '全景参数条·七行',
    desc: '全幅照片压七行参数条：机型、日期、四段曝光参数与坐标，左对齐等宽小字',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_plate_pairs_five',
    group: '经典',
    name: '铭牌·五组字段',
    desc: '白底铭牌五行「字段名 + 数值」对照：左列大写小字标签、右列数值，展签式规格排布',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_datacard_four_pairs',
    group: '经典',
    name: '数据卡·四组对照',
    desc: '白卡四组「数值 + 单位」对照排版，大数值配小单位，左值右标的规格卡',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
  ...
  {
    id: 'm_float_params_right',
    group: '水印署名',
    name: '浮层参数块·右缘五行',
    desc: '照片铺满无边框，右下角五行参数浮层贴右缘收排，上方一条分隔线',
    category: 'frame',
    builtin: true,
    config: {
      bgMode: 'solid',
...
/**
 * 内置模板全量（BUILTIN + 晋升款 + 文字块族）。
 * 注意：**两条返回分支都必须用同一个全量函数**——2026-09-16 加入文字块族时曾漏改「本地已有
 * 自定义模板」这条分支，导致桌面端（本地存过自定义模板）看不到新批次，而全新浏览器正常。
 */
function builtinAllTemplates(): FrameTemplate[] {
  return [...BUILTIN, ...PROMOTED_TEMPLATES, ...TEXT_BLOCK_TEMPLATES]
}
  ...
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed)) {
        ...
        return [...builtinAllTemplates(), ...custom]
      }
    }
  } catch {
    /* ignore */
  }
  return builtinAllTemplates()
}
