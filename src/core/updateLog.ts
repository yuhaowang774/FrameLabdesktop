...
export const UPDATE_LOG: UpdateEntry[] = [
  {
    ...
    groups: {
      added: [
        '月历边框：新增「月历边框」信息布局，年月刊头 + 星期表头 + 公历/农历双历网格（初一切换农历月名），拍摄日期以强调色圆点自动标记；新增「月历·白纸手账 / 暗夜桌历」模板',
        '运动遥测：支持导入 GPX 文件并解析距离 / 时长 / 均速 / 爬升，新增「运动遥测」信息布局（轨迹缩略卡 + 四栏参数表）',
        '相机机身壳：新增黑机身银顶盖 / 银色机身两种自绘矢量机身壳（金属顶盖 + 快门钮 + 模式拨盘 + 皮革握把 + 按钮列），预览与成片一致，配套 8 套机身壳模板',
        '设备样机：照片可装进手机机身轮廓（深空灰 / 银色，含边框环与灵动岛），新增「设备样机」分组与 7 套手机壳模板',
        '杂志封面款：新增「杂志封面·刊头款 / 展览款」模板——全幅照片压衬线大标题 + 报头行，展览款带多行介绍段落',
        '胶片与拍立得：新增黑片基 / 暖褐片基胶片壳与一次成像白框，票根撕线新增「齿孔线」元素',
        '模板库：内置模板扩充至 106 套，每套自带样张；左栏目录点分类即跳到该段，滚到底可继续进入下一个分类',
      ],
      improved: [
        '信息层自由元素支持边缘锚定（贴照片左右缘 / 画布上下缘），封面报头、底部签名条在任意画幅下不再错位；文字元素支持投影，EXIF 模板新增 {date} 日期字段',
      ],
      fixed: [
        '修复模板库左栏点分类后右侧列表「滚不动」：切换视图未把滚动位置归零，且指针停在左栏时滚轮事件无人接手',
        '修复品牌字标在浅色底带上不可见：字标此前不跟随当前品牌、也没有按底色自动着色',
      ],
    },
  },
  {
    ...
    groups: {
      added: [
        '启动自动检查更新：软件启动后会自动检查新版本，发现更新时在右下角弹出提醒卡片，可「立即更新 / 稍后 / 跳过此版本」；首选项「关于」中可关闭（默认开启）',
      ],
      improved: [
        '启动图库大幅提速：缩略图与照片信息改为本地持久缓存（仅首次导入时生成，之后启动直接读取）；实测 13 张照片（含 2 张 9600 万像素）图库全部就绪从约 42 秒降至 0.3 秒内，进入图库几乎瞬时可见',
        '预览与成片更一致：暗角 / 颗粒 / 水印等效果层此前在编辑画布中被照片遮挡、预览不到位，现已与导出成片完全一致；顶层信息元素（模糊条等）也可直接在画布上看到',
        '批量导出更稳：单张照片导出失败不再中断整批，其余照片照常导出',
        '编辑历史更可靠：修复删除照片后历史记录可能「复活」、本地存储写满时历史静默失效等问题；新增历史容量控制，长期使用更稳定',
      ],
      fixed: [
        '修复批量导入 / 启动还原时，当前照片的参数（机型 / 拍摄参数 / 日期）可能被其它照片覆盖并写入历史记录的问题（此前遇到参数「莫名变化」的用户更新本版后恢复正常）',
        '修复快速连续切换照片时画布可能短暂显示错图 / 错位的问题',
        '修复首选项或弹窗打开时，方向键 / 删除键等快捷键误操作主界面的问题',
        '修复面板拖拽（胶片条 / 侧栏宽度）在指针意外丢失后「粘住」、界面持续跟随鼠标的问题',
        '修复缩略图 / 导出预览的图片资源未及时释放导致的内存占用问题',
        '安全加固：本地图片访问收紧为图片格式白名单，外部链接打开仅允许 https',
      ],
    },
  },
  {
    ...
    groups: {
      added: [
        '参数复制 / 粘贴：编辑页底栏「复制参数 / 粘贴参数」把当前照片的相框 / 背景 / INFO 样式复制到剪贴板，可粘贴到其它照片（存在勾选照片时可一并同步），跨窗口使用',
        '胶片条右键菜单：对任意缩略图右键可「复制参数 / 粘贴参数 / 导出该照片」，一键把当前样式复用或导出',
        '编辑画布右键「导出当前照片」：不用切换模块，画布上右键即可快速导出当前照片',
      ],
      improved: [
        '导入与启动图库大幅提速：照片宽高、文件大小改由底层直接解析文件头（毫秒级），并修复了此前快速路径未生效的问题——实测 96MB 大图读取用时从约 0.5 秒降至 2 毫秒，导入大量照片 / 启动还原时明显更快',
        '胶片条缩略图自适应：高度随胶片条拖拽高低伸缩，宽度按每张照片自身比例显示（横图 / 竖图都完整呈现）',
        'INFO 相机型号与日期样式独立控制：分别调字号、字体不再互相影响',
        '图库勾选更稳定：浏览照片（点击、方向键）不再清空已勾选的照片，勾选只由圆圈 / Ctrl+点击 / Shift 范围控制，导出严格按照勾选导出；批量导出未勾选时仅导出当前照片',
        '界面简化：边框设置移除「自由拖拽」模式（只保留原简易模式）；相机型号移除无实际效果的「型号距 Logo」间距项',
      ],
      fixed: [
        '修复导出实际导出的不是勾选照片的问题（此前点缩略图浏览会静默清空勾选）',
        '修复右键菜单时有时无（切换照片时菜单被残留监听立即关闭）',
        '修复「我的模板」应用后部分参数（字体 / 字号 / 粗细 / 透明度）不生效、与模板预览不一致的问题',
        '修复导出勾选圆圈偶尔点不到（被缩略图遮挡）的问题',
      ],
    },
  },
  {
