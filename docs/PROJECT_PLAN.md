...
| 2026-09-14 | **画布触控适配（平板 / 触屏 / 触控笔，官网网页版体验）**：主画布（Workspace stage）新增触屏手势——① **双指缩放**：以两指中点为锚点，与滚轮/双击共用同一套锚点数学（新增纯函数 `core/pinch.ts`：`pinchStep`/`pinchSample`，含 6 项单测；支持缩放同时平移、10%~800% 钳制、到达上下限画面不漂移）；② **双轻点**放大 2x / 复位：合成 dblclick 在嵌入式 WebView 不可靠，自行按「300ms / 30px / 非多指 / 未拖动」判定，并抑制随后到达的合成 dblclick 防重复；③ **双指后剩一指无缝续接单指平移**；④ **`pointercancel` 兜底**：系统手势打断不再残留幽灵平移。其余交互经核查原本即基于 Pointer Events（单指平移 / INFO 元素拖拽 / 裁剪框 / 长按菜单）或原生 range（滑块），触控与触控笔可直接使用。337 测试（+6）/ 构建通过；真机验收：CDP 注入真实触摸/触控笔事件到运行中实例——双指张开 100%→300%（精确 ×3）、捏合回 100%、单指平移与触控笔拖动逐像素跟随、双轻点缩放/复位，全部通过 |
| 2026-09-14 | **模板库样张匹配（55 套模板 × 各自样张照片）+ 冗余模板去重（62 → 55）**：① 从用户提供的 64 张照片中为每套内置模板匹配最贴合其版式气质的样张（高调白框配浅调 / 暗调款配夜景 / 胶片款配胶片感人像 / 旅行刊头配旅途照片，兼顾竖版·方版·9:16 画幅方向），压缩为 `src/assets/template-samples/<模板id>.jpg`（1000px / q80，55 张共 ≈5.2MB；匹配表与再生成入口 `scripts/gen-template-samples.ps1`）；② 新增 `core/templateSamples.ts`（`sampleForTemplate`：文件名即模板 id，缺失自动回退默认示例图/SVG）；③ 模板库卡片统一展示**自身样张**（桌面端与网页版一致，卡片高度随样张比例自然错落；自定义模板无样张时回退当前照片/内置示例图）；列数自适应（宽屏 4 列 / 中屏 3 / 窄屏 2 / 极窄 1 列，响应式；间距收紧至 12px）；④ 缩略图缓存随样张场景扩容（图源 6→16 / 结果 40→80，避免来回滚动反复解码）；⑤ 去重删除 7 套与同系模板高度重合的款式（黑卡金标·联名 / 白卡极简·无日期 / 数据档案·深空 / CCD夜闪·时刻戳 / 多彩·灰紫 / 画册署名·右下 / 大师水印·方幅）。341 测试（+3 样张一一对应守护）/ 构建通过；无头浏览器实测：`verify-samples.html` 总览页 55/55 真实合成、网页版模板库弹窗 55 张卡片全部为真实合成（0 SVG 回退）、侧栏分组计数合计 55 一致 |
| 2026-09-15 | **官网「累计下载」口径修正（A 方案：只统计安装包）**：实测原口径（全部 Release 资产之和 = 113）中 **61% 是更新检查流量**——`latest.json` 69 次 vs 安装包 40 次 vs `.sig` 4 次；且 0.2.8 起「启动自动检查更新」使 latest.json 每次启动都 +1，数字会随启动次数虚增、语义从「下载」变成「启动次数」。改为**只累加 `*.exe` 安装包下载数**（一次安装 / 一次应用内自动更新各计 1 次真实下载，排除 latest.json 与 .sig——签名内嵌在清单里、App 不单独拉 sig），四端同步：`functions/api/gh-stats.js`（代理）+ `scripts/fetch-gh-stats.mjs`（烘焙）+ `website/assets/main.js`（直连兜底）+ `website/index.html` 文案「累计下载（含更新检查）」→「累计下载」。烘焙值刷新为 `{"stars":3,"downloads":40,"version":"v0.2.8"}`；前端缓存 `CACHE_REV` 3→4 立即作废旧口径缓存（避免老访客看到虚高的 113）；`website/docs/PLAN.md`、`TEST-REPORT.md` 口径描述同步 |
| 2026-09-15 | **模板库交互改版：点卡片先预览 → 满意再应用**：点击卡片不再直接应用并关闭弹窗，改为弹出「预览确认层」（覆盖弹窗内容区）——用**当前照片 + 当前 INFO** 走与导出同源的合成管线渲染长边 1600 大图（未打开照片时回退该模板样张并在底部注明），满意点「确认应用」才应用并返回编辑，不满意点「退回继续选择」（或 Esc）回到列表继续挑选；预览态 Enter 直接确认。配套：卡片移除 hover 信息浮层（模板名称/说明只在预览层呈现，列表即纯成片瀑布流）、瀑布流宽屏回到 4 列并保留响应式（递减 3/2/1）、底部提示文案、`AGENTS.md` 设计决策记录（取代 09-13「点卡即应用」）、`TemplatePickerModal.test.ts` 覆盖新流程。343 测试（+3）/ 构建通过；真机验收：dev 端点「白框参数卡」→ 预览层显示本人照片成片（SONY 字标 + E 55-210mm 镜头行 + 实拍参数与日期），加载态/失败兜底与两按钮均正常 |
| 2026-09-15 | **模板库再扩 9 套（用尽全部 64 张照片）+ 新增「纸品印刷」分组（55 → 64 套）**：上网调研流行照片边框范式后**自主实现**（遵守 AGENTS.md 合规边界：只学版式思路，文案/命名/参数/实现全自有）：胶片边码·底片条（深色片基 + 上下压边线 + 两侧竖排边码，颗粒 0.55 / 暗角 0.28）、幻灯片纸框（方形米白厚框 + 框上打印字）、美术馆展签·白墙（大幅下留白 + 衬线作品名 + 器材 provenance 行）、档案索引卡（牛皮纸 + 打字机字段 + 签名线）、明信片·旅行背面（地址横线 + 邮票方框 + 竖分隔线）、手账·日签页（日期抬头 + 随手记 + 右缘竖排）、黑胶封套·SIDE A（方形深色 + 曲目行 + 左缘书脊字）、杂志目录页（CONTENTS 刊头 + 巨型页码 + 章节行）、画廊标题条（全幅照片 + 底部实心标题条）。新增分组「纸品印刷」（4 套）；样张脚本补 9 条映射（用尽剩余 9 张照片：样张 64 张 / 6.2MB，既有 55 张字节未变）。顺带修正 4 处既有模板 EXIF 串重复前缀（`ISO{iso}` → `{iso}`：`{iso}` 展开已自带 ISO）。343 测试 / 构建 / lint 通过；真机验收：胶片边码条、明信片、美术馆展签预览渲染正确（展签放大 2.6 倍核对小字无裁切）|
| 2026-09-15 | **第三批扩充：64→79 套**（用尽新增 15 张照片）。 |
| 2026-09-15 | **模板库列表改为按侧栏分类分段 + 滚动联动预选 + 分区间隔**：右侧「全部」视图按左侧侧栏顺序逐类分段（16 段，段标题 + 64px 间距 + 细分隔线）；左侧侧栏本体固定不动，右侧滑到哪一类，左侧对应分类项轻微高亮（`.tp-item.spy`，与「当前视图」实心高亮区分）。343 测试 / 构建通过 |
| 2026-09-16 | **模板库左栏改为「目录索引」+ 左栏滚轮接力**（用户要求，取代 09-13 起「点分类 = 筛选单类」）：右侧恒为**完整分段列表**（段顺序 = 侧栏顺序：最近使用 → 风格分组 → 其他 → 我的模板），点击左侧分类只把列表**平滑滚到对应段**，因此跳过去之后仍能继续往下滑到下一个分类区域；滚动时左栏对应项高亮指示当前位置；「最近使用 / 我的模板」两段恒在（为空显示引导文案），保证每项都有落点；侧栏「全部」项移除（列表本就恒为全部，套数见顶栏「共 N 套」）；左栏滚轮自身到边界后把余量接力给右侧列表（含「只吃边界余量」处理，避免在边界白吞一格滚轮）。`TemplatePickerModal.test.ts` 同步改写（跳段不筛选 / 段顺序 / 空段引导 / 滚轮接力）。347 测试 / 构建通过；真机验收：滚到 3000px 后点「杂志编辑」→ 列表回到该段顶部（scrollTop 0），随后滚轮继续进入下一分类「胶片复古」，左栏高亮同步跟随 |
| 2026-09-17 | **收尾批：手机壳 7 套 + 社交尺寸 1 套（内置 98 → 106）**：补上此前为空的分组「设备样机」（手机壳引擎早就有、一直没有模板），bottom三行/左缘五行/白带六行/居中七行/居中九行/六组字段/暗调五行，另加 9:16 竖屏两行社交款。348 测试 / vite build 通过 |
| 2026-09-17 | **素材类收尾：片基壳 + 齿孔线 + 7 套（91 → 98）**；**第 3 阶段素材方案 a：自绘相机机身壳 + 机身壳模板 8 套（内置 83 → 91 套）**：语料 `01_相机边框` 的 C 档 8 套原貌是「真实相机照片做底 + 用户照片合成进机身屏幕」的素材拼贴（已逐个看图确认：Leica M 机身背面、Fujifilm X-M5 正面 + 屏幕里再嵌一张），FrameLab 不用第三方素材（AGENTS.md），按约定重释为**矢量机身壳**。引擎侧：`deviceMockup` 由 `'phone-dark'|'phone-light'` 扩到含 `'camera-dark'|'camera-silver'`，`drawDeviceMockup` 按 kind 分派到新的 `drawCameraShell`——照片即机身背面 LCD（环从照片边缘向内收，全幅模板同样适用），环内绘：金属顶盖（热靴凹槽 + 快门钮 + 模式拨盘带刻度）+ 左右皮革握把（横向纹理）+ 右握把按钮列（两枚小圆钮 + 四向拨盘）+ 外缘倒角高光 + LCD 内衬窄边与细框；细节统一 `clip('evenodd')` 裁在环内，圆角/任意画幅下都不越界（环厚 = 长边 4.8%，钳制 20~96px，缩略图与超大导出比例一致）。接线：`BorderSettings` 新增两项（黑机身银顶盖 / 银色机身）+ 壳厚提示；`templateThumb` 的 SVG 兜底同步补相机壳画法。模板侧：`gen-text-block-templates.mjs` 支持 `mockup` 字段与「空 lines = 纯壳体模板」，新增 8 套（联名卡）：黑银全幅 / 银黑全幅 / 方画幅（`frameRatio` 1）/ 品牌三行 / 参数八行 / 左上四行 / 左缘六行（inset 96 让开机身环）/ 带下八行；样张 8 张（图库 79 张照片已被前批占满，本批全部复用，同片看版式差异）。**顺带修**：`frameRatio` 在 `sanitizeTemplateConfig` 里登记为**数值**（宽/高）而非字符串——写 `'1:1'` 会被静默丢弃，已在生成器注释里写明（`ratio: 1`）。348 测试 / `vite build` 通过；真机验收：黑银全幅（顶盖 + 左右握把 + 拨盘齐整）、参数八行（机身壳 + 白底带内居中八行数值/小字标交替）|
| 2026-09-16 | **范式重写 第 4 批：多行文字块族批量 20 套（内置 63 → 83 套）**：把「5~10 行文字块」这一族按规范批量铺开，新增生成器 `scripts/gen-text-block-templates.mjs`（紧凑规格表 → 展开为 `src/core/textBlockTemplates.ts` 冻结字面量，与 promotedTemplates 同形；`useTemplates` 运行时展开进内置清单）。**几何改为推导而非手写**：给出首行距锚点边 `baseY` 与行距 `pitch`，脚本按「首行半高 + 上留白 26 ≤ 带高」算 `borderRatio`，并在块高超出时自动下压保证末行离下缘 ≥ 半高 + 22——第 3 批手调 y 时踩过的「首行顶到带外压在照片上」由此从源头消除（生成时自检告警）。20 套分布：水印署名 +6（浮层规格五行 / 左上八行 / 居中六行 / 左缘十行标签对照 / 大字四行 / 右缘五行）、经典 +4（六行记录 / 居中八行数值 / 七行记录 / 六组字段铭牌）、联名卡 +3（品牌字标 + 五行 / 六行 / 右对齐）、大师水印 +2（居中七行带衬线字与分隔线 / 衬线五行）、创意排版 +2（顶部五行刊头 / 顶部大字两行）、暗调影廊 +1（模糊底五行）、极简轻量 +1（日期三行）。样张：16 张取自尚未占用的用户照片，另 4 张与既有模板同片（便于对比构图差异）。**顺带修两个真 bug**：① `useTemplates.load()` 的「本地已有自定义模板」分支漏拼 `TEXT_BLOCK_TEMPLATES`（新建数组型批次时极易漏）——表现是桌面端（本地存过自定义模板）看不到新批次、全新浏览器却正常；抽 `builtinAllTemplates()` 统一两分支并补回归用例。② infoLayer 的 `logoId: 'brand'`（types.ts 的「跟随当前品牌」约定）在 `useLogoStore` 里从未实现，且绘制的字标没有着色参数 → 白底模板上「白字标压白底」不可见、还会退化成把 id 当文字画（屏幕上出现 "brand" 字样）；现导出端与预览端统一传 `logoAutoColor` + 当前品牌，新增 `logoElementId()` 解析（`'brand'`→当前品牌、`'none'`→跳过）。348 测试 / vite build 通过；真机验收：联名卡款底带内 SONY 字标 + 机型 + 两行参数齐整，大师水印款米白厚带居中七行（衬线机型 + 细分隔线 + 日期），左上八行浮层白字带投影压在照片上 |
| 2026-09-16 | **范式重写 第 3 批：多行文字块范式族 5 套（内置 58 → 63 套）**：先定《多行文字块规范》再落地——字号档（字标 26~30/700、主值 18~20/500~600、小字 14~16/400~500）、行距（逐行元素 pitch 24~36 同块一致、静态多行 lineHeight 1.6~1.9）、字距（拉丁 2~4 / 大写标 4~6 / 中文 1~2）、锚点（左下 = anchorX left + anchorY bottom + y 负；右对齐块用 anchorX right + align right + x 负）、压字规则（照片/模糊底 → shadow: true，纯色带 → 不投影）、数值行一律用 exif 元素（缺字段自动跳过，不出现空行）；规范以注释形式落在 `useTemplates.ts` 第 3 批段落。据此新增：规格卡·五段参数（白底中带 + 五行左对齐：机型/焦段光圈/快门感光/镜头/日期）、铭牌·五组字段（左大写标签 + 右数值 5 组对照）、数据卡·四组对照（大数值 34px + 小单位标 5 组：FOCAL/APERTURE/SHUTTER/SENSITIVITY）、全景参数条·七行（全幅压字：机型/日期/四段曝光/坐标，带投影）、浮层参数块·右缘五行（全幅右下贴右缘收排 + 分隔线）。落库时发现并修正 3 套白边款的文字块几何：底带高度（padding + borderRatio）不足以容纳 5 行块，首行会溢到照片上（深色字压在照片上不可见）——按「块高 + 上下留白 ≤ 带高」重算：spec 带高 170 行距 -168~-40、plate 带高 190 行距 -168~-40、datacard 带高 190 行距 -164~-38。347 测试 / vite build 通过；真机验收：铭牌款五行标签/数值全部落在底带内、LENS 行因照片无镜头数据自动跳过且不留空行，规格卡与浮层款排版正常；样张改用 `-Only` 只生成 5 张 |
| 2026-09-16 | **范式重写 第 2 批：色卡范式族 5 套（内置 53 → 58 套）**：按「先色卡 → 再文字块 → 最后相机壳」的排期先做 engine 现成的色卡族（骨架来源 06_ColorWalk / 07_多彩边框 / 22_色卡边框：底带 + 取色色卡条 + 参数行，全部落到 `infoLayout: 'magazine'` + `showPalette`）：色卡·窄白边（近白窄边 + 只留色卡条，无文字）、色卡·墨底参数（墨底厚带 + hex 色号 + 右侧机型/参数）、色卡·方形白卡（`frameRatio: 1` + 米白 + hex）、色卡·模糊延展带（`bgMode: 'blur'` 底带 + 白字压色卡）、色卡·宽幅白（`frameRatio: 1.5` + 副标题带日期）；分组「多彩色卡」2 → 7 套。样张从用户照片中挑未被占用的 5 张（横幅/竖幅/方形按气质匹配）；`gen-template-samples.ps1` 新增 `-Only` 参数（只生成指定模板样张，避免整目录重刷——本次即用它只产出 5 张，未再带出历史样张）。**顺带修一处预览/导出不同源**：`paletteHex`（色卡 hex 色号）此前只在预览 `FooterInfo` 与 SVG 缩略图绘制，导出端完全没有；且预览端的坐标误用整页坐标（相对容器重复偏移），5 个色号标签重叠在色卡带外侧。现统一为 `infoLayout.ts` 的 `MAG_HEX_SIZE / MAG_HEX_LETTER_SPACING / MAG_HEX_OFFSET_Y` 三常量，导出端 `drawMagazineFooter` 按「每块色卡正下方居中」绘制，预览端改用容器相对坐标（`i * MAG_SWATCH_W`）。347 测试 / vite build 通过；真机验收：墨底款底带上 5 个 hex 色号逐块对齐、模糊延展款白字在模糊带上可读、方形/宽幅款画幅与色卡带比例正常 |
| 2026-09-16 | **FrameElf 范式自主重写 第 1 批 6 套（内置 47 → 53 套）**：先做语料盘点与去重——新增只读扫描器 `scripts/scan-frameelf-paradigms.mjs`（提取画布档/排布/信息区方位厚度/文字骨架/分割线/装饰层构成，聚类并按「需要什么能力」分 A/B/C/D 档）与报告 `docs/frameelf-范式清单.md`（102 套免费模板 → 97 个骨架簇，档位 A15/B68/C19/D0，能力需求 TOP：品牌 Logo 33 套现成、浮层水印 30 套现成、相机壳 15 套需自绘、5~8 行文字块 29 套需自由元素拼装）。第 1 批按 A 档自主重写 6 套（合规：只沿用版式骨架，命名/说明/文案/配色/几何数值全自有）：浮层署名·左缘单行 + 浮层日期·右下角（`infoLayer` 自由元素 + 边缘锚定/投影，新增「水印署名」分组承载）、白边·左对齐双行 + 白边·参数四行（classic 左对齐 + 顶部/底部锚点，两行／四行）、色卡·底部参数带（magazine + paletteHex）、票根·地名字标横版（砖红外圈 + 米白卡 + 照片内缩 + 三元素票据排版）。样张从用户「模版照片」中优先选**未被现有模板占用**的 6 张（横/方/竖按画幅匹配；`$MAP` 追加第 4 批映射并重跑脚本），同时清理脚本重跑带出的 32 张历史样张（保留 5 张 `rc_*` alias 样张）。347 测试 / vite build 通过；真机验收：6 套卡片均为真实合成缩略图，预览层实测「浮层署名」压白字清晰、「白边·参数四行」四行左对齐（机型/参数/镜头/日期，参数区等宽字）、「票根」砖红内圈 + 地名标 + 日期 + 编号排布正确 |
